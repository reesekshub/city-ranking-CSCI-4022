...
OUTPUT_DIR = os.path.join("data", "modality_deltas")


def _read_time_csv(path: str) -> tuple[list[str], list[str], list[list]]:
    """Returns (col_headers, row_labels, value_matrix) from a travel time CSV."""
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            values.append([int(v) if v != "" else None for v in row[1:]])
    return col_headers, row_labels, values


def compute_deltas() -> None:
    """
    For each city in INPUT_DIR, computes t_bicycling - t_driving for every
    ...
    for city in cities:
        ...
        with open(out_path, "w", newline="", encoding="utf-8") as f:
            ...
            for label, bike_row, drive_row in zip(row_labels, bike_vals, drive_vals):
                delta_row = [
                    b - d if (b is not None and d is not None) else ""
